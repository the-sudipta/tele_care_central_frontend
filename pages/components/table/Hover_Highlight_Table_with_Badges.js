...
export default function Hover_Highlight_Table_with_Badges(props) {
                        ...
                        <tr key={rowIndex} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-100 hover:text-black">
                            {props.columns.map((columnName, columnIndex) => (
                                <td key={columnIndex} className="px-6 py-4">
                                    {/* Check if columnName is "status" and get the badge */}
                                    {columnName === "status" ? getStatusBadge(rowData[columnName]) : rowData[columnName]}
                                </td>
                            ))}
                        </tr>
                    ))}
